...
account_key_names = ["id", "username", "acct", "display_name", "locked", "bot", "discoverable", "group", "created_at", "note", "url", "avatar", "avatar_static", "header", "header_static", "followers_count", "following_count", "statuses_count", "last_status_at", "emojis","fields", "queried_at"]
...
def acct_to_string(acct):
    if not "@" in acct["acct"]:
        regex = r"https?:\/\/(.*?)\/@(.*)"
        try:
            instance, name = re.search(regex, acct["url"], re.MULTILINE).groups()
        except:
            return acct["url"]
...
def aggregate_timelines(timelines):
    ...
    for uri, toots in unique_toots.items():
        if len(toots) == 1:
            ...
            toot = sorted([t for i, t in toots], key=lambda t: (t["replies_count"]+t["reblogs_count"]+t["favourites_count"]), reverse=True)[0]
...
def filter_toots(toots, query=None, notes_tags = ["nosearch", "nobots", "noindex", "nobot"]):
    ...
    if notes_tags:
        regex = re.compile(f"#<span>({'|'.join(notes_tags)})<\/span>", re.IGNORECASE)
        filtered_toots = []
        for toot in toots:
...
def sanitize_toot(raw_toot, instance_name = None, parse_html = False):
    ...
    if "edited_at" in raw_toot.keys() and raw_toot["edited_at"] is not None:
        try:
            toot["edited_at"] = datetime.strptime(f"{raw_toot['edited_at']} -0000", "%Y-%m-%dT%H:%M:%S.%fZ %z")
    ...
    try:
        ...
        toot["user_created_at"] = raw_toot["account"]["created_at"].replace(".000Z", "")
    ...
    try:
        try:
            ...
            toot["user_last_status_at"] = raw_toot["account"]["last_status_at"].replace(".000Z", "")
    except:
    ...
    if queried_instances:
        for data in queried_instances.values():
            ...
            try:
                ...
                if parse_html:
                    instance["short_description"] = parse_toot_html(instance["short_description"])
            except:
            ...
            try:
                ...
                if parse_html:
                    instance["description"] = parse_toot_html(instance["description"])
            except:
            ...
            try:
                instance["weekly_statuses"] = int(sum([a["statuses"] for a in activity[1:]])/len(activity[1:]))
                instance["weekly_logins"] = int(sum([a["logins"] for a in activity[1:]])/len(activity[1:]))
                instance["weekly_registrations"] = int(sum([a["registrations"] for a in activity[1:]])/len(activity[1:]))
            except:
    ...
    if queried_toots:
        for toot in queried_toots:
            if type(toot) is list:
                for t in toot:
                    try:
                        sanitized_toot = sanitize_toot(t, parse_html = parse_html, instance_name = instance_name)
    ...
    if queried_toots:
        with open(file_name, file_mode, newline='') as f:
            ...
            if not append:
                ...
                writer.writerow(key_names)
            parsed_toots = toots_to_lines(queried_toots, parse_html = parse_html, instance_name = instance_name)
...
def get_toots_reblogs(toots, request_timeout = 15, verbose = False):
    ...
    for t in toots:
        ...
        logger.info(f"Retrieved {len(reblogs[t['uri']]['reblogs'])} reblogs for {t['uri']}")
...
def get_toots_favourites(toots, request_timeout = 15, verbose = False):
    ...
    for t in toots:
        ...
        logger.info(f"Retrieved {len(favs[t['uri']]['favourites'])} favourites for {t['uri']}")
    ...
    for t in toots:
        ...
        try:
            ...
            if new_page:
                ...
                context[t["uri"]]["descendants"].extend(new_page["descendants"])
            while new_page:
                new_page = api.fetch_next(new_page)
                if new_page:
                    context[t["uri"]]["ancestors"].extend(new_page["ancestors"])
        ...
        logger.info(f"Retrieved {len(context[t['uri']]['ancestors'])} ancestors and {len(context[t['uri']]['descendants'])} descendants for {t['uri']}")
...
def get_account_followers(account_url, max_followers=None, verbose = False, request_timeout = 15):
    ...
    if max_followers:
        logger.info(f"Getting {max_followers} followers for {account['url']} ({account['followers_count']} followers)")
    else:
        max_followers = int(account['followers_count'])
        logger.info(f"Getting all followers for {account['url']} ({account['followers_count']} followers)")
    ...
    while paginate:
        ...
        if new_followers:
            ...
            if len(queried_accounts) < max_followers and len(new_followers) > 0:
                logger.debug(f"Retrieved {len(new_followers)} new followers and {len(queried_accounts)} followers in total")
            else:
                paginate = False
        else:
            paginate = False
...
def get_accounts_by_url(urls, file_name = None, parse_html = False,request_timeout = 15, verbose = False):
    ...
    for url in urls:
        ...
        logger.info(f"Retrieved info for @{account['acct']} \"{account['display_name']}\" ({account['followers_count']} follower, {account['statuses_count']} posts)")
...
def search_public(api_base, query = None, access_token = None, min_id = None, max_id = None, max_toots = None, local_only = False, verbose = False, request_timeout = 30):
    ...
    try:
        if min_id and not max_id:
            new_toots = api.timeline_public(limit=40, min_id = min_id, local = local_only)
        elif max_id and not min_id:
            new_toots = api.timeline_public(limit=40, max_id = max_id, local = local_only)
...
def search_hashtag(queried_hashtag, api_base, access_token = None, min_id = None, local_only = False, verbose = False, request_timeout = 30):
    ...
    queried_toots = []
    try:
        new_toots = api.timeline_hashtag(hashtag = queried_hashtag, limit=40, local = local_only, min_id = min_id)
        queried_toots.extend(add_queried_at(new_toots))
    except mastodon.MastodonAPIError as e:
        logger.warning(f"There was a problem connecting to {api_base}: {e.args[1:]}")
        return queried_toots
